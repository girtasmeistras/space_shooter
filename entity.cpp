#include <iostream>
#include "entity.h"
std::vector<std::shared_ptr<Entity>> Entity::entity_list;
...
void Entity::check_for_collisions() {
    ...
    for(unsigned int i = 0;i < entity_list.size();i++) {
        if(this != entity_list[i].get() && entity_list[i].get() != nullptr && alive && entity_list[i]->is_alive() == true &&collides(entity_list[i]->entity_pos) == true) {
